...
def feature_candidates(preparation: DataPreparation, target: str) -> list[str]:
    ...
    for name, descriptor in preparation.schema_json.items():
        ...
        if normalized in {"id", "index", "row_id", "rowid", "uuid"} or normalized.endswith("_id"):
            continue
        types = descriptor.get("types") or {}
...
def build_classification_baseline(
    ...
    group_field = next(
        (
            ...
            if any(marker in name.casefold() for marker in ("source", "type", "category", "domain"))
        ),
        None,
    )
...
def mcnemar_test(y_true, model_pred, baseline_pred):
    model_only = sum(
        1 for truth, model, baseline in zip(y_true, model_pred, baseline_pred)
        if model == truth and baseline != truth
    )
    baseline_only = sum(
        1 for truth, model, baseline in zip(y_true, model_pred, baseline_pred)
        if model != truth and baseline == truth
    ...
    if discordant == 0:
        return 1.0, 0.0, model_only, baseline_only
    statistic = ((abs(model_only - baseline_only) - 1) ** 2) / discordant
    p_value = math.erfc(math.sqrt(statistic / 2))
    return max(0.0, min(1.0, p_value)), statistic, model_only, baseline_only
...
for seed in SEEDS:
    ...
    if GROUP_FIELD:
        ...
        for row_tokens, label in train:
            group_tokens = [
                ...
                if token.startswith(GROUP_FIELD + "=") or token.startswith(GROUP_FIELD + ":")
            ]
            group = group_tokens[0] if group_tokens else "__missing__"
            grouped.setdefault(group, []).append(label)
        group_labels = {{
            group: Counter(values).most_common(1)[0][0]
    ...
    y_group_base = []
    for row_tokens, _label in test:
        group_tokens = [
            token for token in row_tokens
            if GROUP_FIELD
            and (token.startswith(GROUP_FIELD + "=") or token.startswith(GROUP_FIELD + ":"))
        ]
        group = group_tokens[0] if group_tokens else "__missing__"
        y_group_base.append(group_labels.get(group, majority))
    acc = accuracy(y_true, y_pred)
    base_acc = accuracy(y_true, y_base)
    group_base_acc = accuracy(y_true, y_group_base)
    f1 = macro_f1(y_true, y_pred)
    accuracies.append(acc)
    baseline_accuracies.append(base_acc)
    group_baseline_accuracies.append(group_base_acc)
    strongest_baseline = y_group_base if group_base_acc >= base_acc else y_base
    all_true.extend(y_true)
    all_model_predictions.extend(y_pred)
    all_baseline_predictions.extend(strongest_baseline)
    all_differences.extend([
        (1.0 if true == pred else 0.0) - (1.0 if true == base else 0.0)
        for true, pred, base in zip(y_true, y_pred, strongest_baseline)
    ])
    per_seed_metrics.append({{
        "seed": seed,
        "metrics": {{
            ...
            "group_baseline_accuracy": group_base_acc,
            "macro_f1": f1,
            "test_rows": len(test),
        }},
...
group_baseline_accuracy = mean(group_baseline_accuracies)
lower, upper = bootstrap_interval(accuracies, 42)
p_value, statistic, model_only, baseline_only = mcnemar_test(
    all_true, all_model_predictions, all_baseline_predictions
)
effect = cohens_d(all_differences)
result = {{
    "num_samples": len(examples),
    ...
    "per_seed_metrics": per_seed_metrics,
    "baseline_metrics": {{
        "majority_class": {{"accuracy": baseline_accuracy}},
        "group_conditioned_majority": {{
    ...
    "statistical_test": {{
        "name": "mcnemar_continuity_corrected",
        "statistic": statistic,
        "p_value": p_value,
...
'''
    ...
    return ExperimentDraft(
        ...
        metrics=["accuracy", "macro_f1", "95% bootstrap interval", "McNemar test"],
        methodology=[
            "Use the selected label field from the prepared dataset.",
            "Evaluate three fixed random train/test splits.",
            "Compare Naive Bayes against global and source-conditioned majority baselines.",
        ...
        scientific_plan={
            ...
            "split_strategy": "80/20 train/test split repeated with seeds 42, 43, and 44",
            "baselines": ["train majority class", "source-conditioned train majority class"],
            "metric_definitions": {
            ...
            "statistical_analysis": (
                "bootstrap interval over seed-level accuracy and continuity-corrected "
                "McNemar test over paired held-out predictions"
            ),
            "seeds": [42, 43, 44],
            "parameters": {"test_fraction": 0.2},
            "expected_sample_count": preparation.row_count,
